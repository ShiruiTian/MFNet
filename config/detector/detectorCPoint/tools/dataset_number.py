...
class KittiDataset(torch_data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # sample_info = {'sample_id': sample_id, 'frame_id': index}
        sample_info = {'sample_id': sample_id, 'frame_id': sample_id}
    ...
    def collate_batch(self, batch):
        ...
        for key in batch[0].keys():
            ...
            if key == 'foreground_pts':
                for k in range(batch_size):
                    # frame_foreground_pts.append(batch[k][key])
                    # frame_foreground_pts.append(batch[k]['sample_id'])
                    frame_foreground_pts.append(batch[k]['foreground_pts'])
                    sample_ids.append(batch[k]['sample_ids'])
                    # frame_ids.append(batch[k]['sample_id'])
                    # sample_ids.append(batch[k][0]['sample_id'])
                # frame_foreground_pts = [item[key] for item in batch]
 
    
        # ans_dict['sample_id'] = sample_id
        ans_dict['sample_ids'] = sample_ids
        ans_dict['foreground_pts'] = frame_foreground_pts  
...
'''    def collate_batch(self, batch):
        ...
        for key in batch[0].keys():
            ...
            if key == 'foreground_pts':
                for k in range(batch_size):
                    frame_foreground_pts.append(batch[k][key])
                    frame_ids.append(batch[k]['frame_id'])  
    
        ans_dict['frame_ids'] = frame_ids  
