...
def draw_objs_new(image: Image,
                  predictions: list = None,  # predictions:(box,class,score)
                  category_index: dict = None,
    ...
    colors = [ImageColor.getrgb(STANDARD_COLORS[pre[1] % len(STANDARD_COLORS)]) for pre in predictions]

    if draw_boxes_on_image:
        # Draw all boxes onto image.
        draw = ImageDraw.Draw(image)
        for pre, color in zip(predictions, colors):
            left, top, right, bottom = pre[0]
            # 绘制目标边界框
            draw.line([(left, top), (left, bottom), (right, bottom),
                       (right, top), (left, top)], width=line_thickness, fill=color)
            # 绘制类别和概率信息
            draw_text(draw, pre[0], int(pre[1]), float(pre[2]), category_index, color, font, font_size)

    return image


def draw_obj(image: Image,
             boxes: np.ndarray = None,
             color='Red', width=3):
    # Draw all boxes onto image.
    draw = ImageDraw.Draw(image)
