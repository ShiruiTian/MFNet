...
class Matcher:
    ...
    def match(cls, state_list, measure_list):  # 两个list中单个元素数据结构都是[c_x, c_y, w, h].T
        ...
        :param measure_list: 量测list (box1, class1, score1)
    ...
    @classmethod
    def cal_iou(cls, state, measure, alpha=1):  # alpha: 调整类别与 iou的比例
        """
        计算状态和观测之间的IOU
        ...
        """
        score = 0
        if state.class1 == measure[1]:
            score += alpha

        state_box = utils.mea2box(state.X_posterior[0:4])  # [lt_x, lt_y, rb_x, rb_y].T
        measure_box = utils.mea2box(measure[0])  # [lt_x, lt_y, rb_x, rb_y].T
        s_tl_x, s_tl_y, s_br_x, s_br_y = state_box[0], state_box[1], state_box[2], state_box[3]
        m_tl_x, m_tl_y, m_br_x, m_br_y = measure_box[0], measure_box[1], measure_box[2], measure_box[3]
        # 计算相交部分的坐标
        ...
        if inter == 0:
            ...
            return score + inter / ((s_br_x - s_tl_x) * (s_br_y - s_tl_y) + (m_br_x - m_tl_x) * (m_br_y - m_tl_y) - inter)
